...
Status: lauffähige Backend-Basis mit Fastify/TypeScript, Prisma 7.10 und PostgreSQL 17, Migrationen, Environment-Validierung, JSON-Logging, zentraler Fehlerbehandlung und `/health`. Authentifizierung mit zwei passwortlosen Accounts, mehreren Geräten, Einmal-Pairing und Credential-Widerruf ist implementiert. Album-Metadaten, atomarer Original-Upload sowie asynchron erzeugte Bild-/Video-Derivate mit Range-Streaming sind implementiert. Originale bleiben unverändert; Jobstatus, automatische Wiederholung und manueller Retry liegen in PostgreSQL. Die installierbare native Android-App konfiguriert den Server, verbindet Geräte per Setup oder Pairing und zeigt lokale MediaStore-Alben. Ausgewählte Alben werden über eine persistente Room-/WorkManager-Queue inventarisiert und als unveränderte Originale idempotent hochgeladen. Der Partnerzugriff ist serverseitig an den Freigabestatus gebunden; die Partnergalerie fehlt noch.
...
Album-, Asset-, Upload- und Variantenverträge stehen in der [API-Dokumentation](docs/api.md). Das konkrete Auflösungs-/Codecprofil und die Qualitätsmessung stehen unter [Medien-Derivate](docs/media-derivatives.md). Uploads verwenden zuerst JSON-Metadaten und danach `PUT /v1/assets/{id}/original` mit `application/octet-stream` und exakter `Content-Length`.
...
Eine separat erreichbare PostgreSQL-17-Instanz bereitstellen. Die Compose-DB veröffentlicht absichtlich keinen Host-Port. In `server/.env` die Verbindungsdaten und zwei absolute Medienpfade eintragen; ausgewähltes Verzeichnis vorher anlegen.
