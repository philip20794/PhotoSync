...
test('real PostgreSQL: applied migration, ready health, missing baseline and recovery', async (t) => {
  ...
  assert.ok(migrations.some((row) => row.migration_name === '20260925000100_account_login'));
  ...
  try {
    ...
    await database.client.serviceMetadata.create({ data: { key: 'schema_version', value: '13' } });
  }
  assert.equal((await app.inject('/health')).statusCode, 200);
});
