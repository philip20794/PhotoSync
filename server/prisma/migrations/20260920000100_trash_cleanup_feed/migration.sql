...
BEGIN
  ...
  IF TG_TABLE_NAME = 'albums' AND TG_OP = 'UPDATE'
    ...
    AND (old_item->'sharedAt') IS NOT DISTINCT FROM (new_item->'sharedAt') THEN
    RETURN NULL;
  END IF;
  ...
  IF TG_TABLE_NAME = 'assets' THEN
    ...
    IF TG_OP = 'UPDATE' THEN
      IF old_item->>'status' = 'ready' AND new_item->>'status' = 'deleted' THEN
        NULL;
      ELSIF old_item->>'status' = 'deleted' AND new_item->>'status' = 'ready' THEN
        NULL;
      ELSIF new_item->>'status' <> 'ready' THEN
        RETURN NULL;
      ELSIF old_item->>'status' = 'ready'
        AND (old_item - ARRAY[
          'updatedAt','uploadStartedAt','uploadLeaseId','uploadLeaseExpiresAt',
          'cleanupAttempts','cleanupLastError','cleanupRequestedAt','purgeAfter','purgedAt'
        ]::text[])
  ...
  IF TG_TABLE_NAME = 'asset_derivatives' THEN
    SELECT status INTO asset_status FROM assets WHERE id = (item->>'assetId')::uuid;
    IF asset_status IS DISTINCT FROM 'ready' THEN RETURN NULL; END IF;
    IF TG_OP = 'INSERT' AND new_item->>'status' <> 'ready' THEN RETURN NULL; END IF;
    IF TG_OP = 'DELETE' AND old_item->>'status' <> 'ready' THEN RETURN NULL; END IF;
    IF TG_OP = 'UPDATE' THEN
  ...
  operation := CASE
    WHEN TG_TABLE_NAME = 'assets' AND TG_OP = 'UPDATE'
      AND old_item->>'status' = 'ready' AND new_item->>'status' = 'deleted' THEN 'DELETE'
    WHEN TG_TABLE_NAME = 'assets' AND TG_OP = 'UPDATE'
      AND old_item->>'status' = 'deleted' AND new_item->>'status' = 'ready' THEN 'RESTORE'
